...
if (in_array($action, ['create', 'join'], true)) {
    $boothSecrets = require __DIR__ . '/../booth-secret.php';
    $boothSecret = (string)($boothSecrets['BOOTH_SECRET'] ?? '');
    $boothPass = (string)($input['pass'] ?? '');
    if ($boothSecret === '' || $boothPass === '') respond(403, ['error' => 'Booth pass required']);
    $passParts = explode('.', $boothPass);
    if (count($passParts) !== 2) respond(403, ['error' => 'Malformed booth pass']);
    [$passPayloadB64, $passSignature] = $passParts;
    $expectedSignature = hash_hmac('sha256', $passPayloadB64, $boothSecret);
    if (!hash_equals($expectedSignature, $passSignature)) respond(403, ['error' => 'Invalid booth pass signature']);
    $passJson = base64_decode(strtr($passPayloadB64, '-_', '+/'), true);
    $passPayload = $passJson !== false ? json_decode($passJson, true) : null;
    if (!is_array($passPayload)) respond(403, ['error' => 'Malformed booth pass payload']);
    if (!isset($passPayload['exp']) || time() > (int)$passPayload['exp']) respond(403, ['error' => 'Booth pass expired']);
    if (!isset($passPayload['booth']) || $passPayload['booth'] !== 'ipv1') respond(403, ['error' => 'Booth pass is for a different booth']);
    if (!isset($passPayload['seats']) || (int)$passPayload['seats'] < 2) respond(403, ['error' => 'Booth pass has insufficient seats']);
    if (!isset($passPayload['jti']) || !is_string($passPayload['jti']) || !preg_match('/^[a-f0-9]{32}$/', $passPayload['jti'])) respond(403, ['error' => 'Booth pass missing or invalid jti']);
}
