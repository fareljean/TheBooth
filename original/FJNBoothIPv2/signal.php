...
const LIMIT=4; const TTL=21600; const STALE=45; const MAX_SIGNALS=500; const MAX_BODY=262144;
...
/** Drop ghost seats that stopped polling so the same room code can be rejoined while the booth is active. */
function prune(array &$r):bool{$now=time();$changed=false;if($now-(int)($r['updated']??$r['created']??$now)>TTL){$r['participants']=[];return true;}foreach($r['participants']as$pid=>$v){if($now-(int)($v['seen']??0)>STALE){$seat=(int)($v['seat']??0);unset($r['participants'][$pid]);signal($r,'peer-left',$pid,null,['seat'=>$seat,'stale'=>true]);$changed=true;}}return$changed;}
if(($_SERVER['REQUEST_METHOD']??'GET')==='OPTIONS')out(204,[]);$a=(string)($_GET['action']??'');$b=($_SERVER['REQUEST_METHOD']??'')==='POST'?input():[];
if($a==='ping')out(200,['ok'=>true,'build'=>'FJN-Booth-V2-Four-Seat','stale'=>STALE]);
...
if($a==='join')locked($dir,$c,function($p,$r)use($c){if(!$r)out(404,['error'=>'Room not found']);prune($r);if(!$r['participants']){@unlink($p);out(404,['error'=>'Room not found']);}save($p,$r);if(count($r['participants'])>=LIMIT)out(409,['error'=>'Room full','full'=>true]);$used=array_column($r['participants'],'seat');$seat=1;while(in_array($seat,$used,true))$seat++;$id=peer_id();$t=token();$r['participants'][$id]=['seat'=>$seat,'token'=>hash('sha256',$t),'seen'=>time()];signal($r,'peer-joined',$id,null,['seat'=>$seat]);save($p,$r);out(200,['ok'=>true,'room'=>$c,'peer'=>$id,'seat'=>$seat,'token'=>$t]);});
...
if($a==='poll'){$after=max(0,(int)($_GET['after']??0));locked($dir,$c,function($p,$r)use($id,$t,$after){if(!$r)out(404,['error'=>'Room not found']);if(!role($r,$id,$t))out(403,['error'=>'Denied']);prune($r);if(!isset($r['participants'][$id])){save($p,$r);out(410,['error'=>'Seat expired']);}$r['participants'][$id]['seen']=time();$s=array_values(array_filter($r['signals'],fn($x)=>(int)$x['id']>$after&&($x['to']===null||$x['to']===$id||$x['from']===$id)));save($p,$r);$people=[];foreach($r['participants']as$pid=>$v)$people[]=['id'=>$pid,'seat'=>$v['seat']];out(200,['ok'=>true,'participants'=>$people,'signals'=>$s]);});}
